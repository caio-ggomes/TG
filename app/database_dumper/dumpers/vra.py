...
class VraDumper(Dumper):
    ...
    def _id_voo(self) -> pd.Series:
        return self.sample['icaoempresa'] + self.sample['numvoo'].astype(str)

    @cached_property
    def _horario_partida(self) -> pd.Series:
        return pd.to_datetime(self.sample['partidareal']) + pd.Timedelta('3h')
            ...
            column_values.update(
                {
                    ...
                    # f'{column_family}:{column}'.encode(): (
                    #     self.sample.loc[row, column].encode()
                    #     if isinstance(self.sample.loc[row, column], str)
                    #     else self.sample.loc[row, column].tobytes
                    # )
